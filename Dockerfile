...
RUN go build -v -o hyperosBackendDemo cmd/main.go
...
# IMAGE TARGETS
# -------------
FROM gengweifeng/gcr-io-distroless-static-nonroot as hyperosBackendDemo
WORKDIR /
COPY --from=builder /workspace/hyperosBackendDemo .
USER 65532:65532
EXPOSE 8000
ENTRYPOINT ["/hyperosBackendDemo"]
